...
function ensureSummaryFile(): SummaryFile {
  try {
    if (!fs.existsSync(SUMMARY_PATH)) {
      const initial: SummaryFile = {
        events: [],
        processedTimestamps: [],
...
export async function saveSummaryEvent(event: SummaryEvent) {
  const data = ensureSummaryFile();

  if (event.blockNumber <= data.highestProcessedBlock) return;
 ...
 */
export async function commitSummaryPosted(timestampMs: number) {
  const data = ensureSummaryFile();
...
export async function loadSummaryStore(): Promise<SummaryFile> {
...
async function wasSummaryProcessed(timestampMs: number): Promise<boolean> {
  const data = ensureSummaryFile();
  return data.processedTimestamps.includes(timestampMs);
}

export async function markSummaryProcessed(timestampMs: number) {
  const data = ensureSummaryFile();
  if (!data.processedTimestamps.includes(timestampMs)) {
    data.processedTimestamps.push(timestampMs);
  }
...
export async function shouldPostSummary(): Promise<{ shouldPost: boolean; timestamp: Date }> {
  const ts = await getLatestSeenTimestamp();
  if (!ts) return { shouldPost: false, timestamp: new Date(0) };
...
export async function getLatestSeenTimestamp(): Promise<Date | null> {
...
function getLastTuesdayAt16UTC(ref: Date): Date {
